...
从 0.4.45 开始，`修复 cache 类型行` 还会确保生成的 xlsx 具备 ExcelToSO/ExcelDataReader 可读的 OpenXML 部件（`sharedStrings.xml`、`styles.xml`）。如果旧 cache 是极简 inline string 形态，修复会离线重写为兼容形态；若 postflight 仍失败，Desktop 不会进入导入 Unity。
